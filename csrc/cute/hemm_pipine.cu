...
// gemm_multi_stage(void* Dptr, const void* Aptr, const void* Bptr, int m, int
// n,
//                  int k) {
...
//   auto tCrA = thr_mma.partition_fragment_A(gA(_, _, 0));  // (MMA, MMA_M,
//   MMA_K) auto tCrB = thr_mma.partition_fragment_B(gB(_, _, 0));  // (MMA,
//   MMA_N, MMA_K) auto tCrD = thr_mma.partition_fragment_C(gD);           //
//   (MMA, MMA_M, MMA_N)
...
//   kStage) auto tCrA_view = s2r_thr_copy_a.retile_D(tCrA);  // ? (CPY, CPY_M,
//   CPY_K)

//   auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
//   auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
//   auto tBsB = s2r_thr_copy_b.partition_S(sB);  // ? (CPY, CPY_M, CPY_K,
//   kStage) auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);  // ? (CPY, CPY_M,
//   CPY_K)
...
//   int ik = 0;
//   // smem -> reg
//   cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, ismem_read), tCrA_view(_, _,
//   ik)); cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, ismem_read), tCrB_view(_,
//   _, ik));
...
// template <typename T_, int kTileM_ = 128, int kTileN_ = 128, int kTileK_ =
// 32,
...
//   auto t32x32_cublaslt = local_tile(tD_host_cublaslt, tile, make_coord(0,
//   0));
