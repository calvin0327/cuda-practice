...
def get_cache_swizzle_autotune_config():
    return [
        triton.Config(
            {
                "BLOCK_SIZE_M": 128,
...
@triton.autotune(configs=get_cache_swizzle_autotune_config(), key=["M", "N", "K"])
@triton.jit
def matmul_cache_swizzle_kernel(
    a_ptr,
    b_ptr,
    c_ptr,
    M,
    ...
    gourp_size_m = min(num_pid_m - first_pid_m, GROUP_SIZE_M)

    pid_m = (pid % num_pid_in_group) % gourp_size_m
    pid_n = (pid % num_pid_in_group) // gourp_size_m
...
def matmul_cache_swizzle(
    ...
    matmul_cache_swizzle_kernel[grid](
        a,
        b,
        c,
        M,
...
def test():
    ...
    # Test matmul_cache_swizzle
    torch.manual_seed(0)
    a = torch.randn((512, 512), device="cuda", dtype=torch.bfloat16)
    b = torch.randn((512, 512), device="cuda", dtype=torch.bfloat16)

    triton_out = matmul_cache_swizzle(a, b)
    torch_out = torch.matmul(a, b)
    print(f"triton_cache_swizzle_output_with_fp16_inputs={triton_out}")
    print(f"torch_output_with_fp16_inputs={torch_out}")
    ...
    triton.testing.assert_close(
        ...
        torch_out,
        rtol=1e-4,
        err_msg=f"Triton cache swizzle and Torch differ",
    )
    ...
    if TORCH_HAS_FP8 and utils.is_cuda():
        ...
        # Test matmul_cache_swizzle with FP8 inputs
        torch.manual_seed(0)
        a = torch.randn((512, 512), device="cuda", dtype=torch.bfloat16)
        b = torch.randn((512, 512), device="cuda", dtype=torch.bfloat16)
        a = a.to(torch.float8_e5m2)
    ...
    for fp8_inputs in [False, True]:
        ...
        configs.append(
            triton.testing.Benchmark(
                ...
                line_arg="provider",
                line_vals=(
                    ["triton", "triton (cache swizzle)"]
                    if fp8_inputs
                    else [ref_lib.lower(), "triton", "triton (cache swizzle)"]
                ),
                line_names=(
                    ["Triton", "Triton (cache swizzle)"]
                    if fp8_inputs
                    else [ref_lib, "Triton", "Triton (cache swizzle)"]
                ),
...
def benchmark(M, N, K, provider, fp8_inputs):
    ...
    if provider == "triton (cache swizzle)":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul_cache_swizzle(a, b), quantiles=quantiles
        )
